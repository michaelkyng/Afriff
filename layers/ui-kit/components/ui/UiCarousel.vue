...
<template>
  <div class="relative">
    <ul
      ref="track"
      :aria-label="label"
      class="scrollbar-none -mx-4 flex snap-x snap-mandatory scroll-px-4 gap-4 overflow-x-auto px-4 pb-2 md:mx-0 md:scroll-px-0 md:px-0"
    >
      <slot />
    </ul>
    <template v-if="canScroll">
      <button
        type="button"
        class="absolute top-[38%] -left-3 hidden size-10 -translate-y-1/2 place-items-center rounded-full border border-line bg-surface shadow-lift transition-opacity md:grid"
      ...
      <button
        type="button"
        class="absolute top-[38%] -right-3 hidden size-10 -translate-y-1/2 place-items-center rounded-full border border-line bg-surface shadow-lift transition-opacity md:grid"
